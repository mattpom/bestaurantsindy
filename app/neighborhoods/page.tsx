...
const areas = [
  {
    id: "downtown",
    name: "Downtown",
    intro: "The Indianapolis essentials: historic dining rooms, serious steaks, and polished places built for a night that matters.",
    anchor: "St. Elmo Steak House",
    restaurants: [
  ...
  },
  {
    id: "mass-ave",
    name: "Mass Ave",
    intro: "Lively, walkable, and useful when a group cannot agree on what it wants—tacos, fried chicken, cocktails, or something more polished.",
    anchor: "Bakersfield",
    restaurants: [
  ...
  },
  {
    ...
    intro: "Fountain Square and neighboring Fletcher Place share one food corridor. The boundary matters on a map; it matters less when deciding where to eat.",
    anchor: "Bluebeard",
    restaurants: [
      {name:"Bluebeard", label:"NEAR FOUNTAIN SQUARE · FLETCHER PLACE", best:"Best for: a changing menu and one of Indy’s strongest kitchens", note:"Contemporary American cooking in a converted warehouse, supported by Amelia’s bread and a menu that rewards curiosity.", price:"$$$", href:"https://bluebeardindy.com/"},
  ...
  },
  {
    id: "broad-ripple",
    name: "Broad Ripple",
    intro: "More than bars: long-running neighborhood institutions, patios, brunch, Italian dinners, and restaurants that work on an ordinary Tuesday.",
    anchor: "Ambrosia",
    restaurants: [
  ...
  },
  {
    id: "northside",
    name: "Northside",
    intro: "College Avenue favorites and destination dining farther north—places that justify leaving downtown and making a reservation.",
    anchor: "Northside Social & Delicia",
    restaurants: [
...
export const metadata={
  title:"Best Restaurants by Indianapolis Neighborhood | BestaurantsIndy",
  description:"Curated Indianapolis restaurant picks for Downtown, Mass Ave, Fountain Square, Broad Ripple, and the Northside."
};

export default function Neighborhoods(){
  return <main>
    <SiteHeader/>
    <section className="pageHero neighborhoodHero">
      <p className="eyebrow">BESTAURANTSINDY NEIGHBORHOOD PICKS</p>
      <h1>Start close. Drive farther if it earns it.</h1>
      <p>Not a scraped ranking and not a directory of every restaurant in town. These are useful starting points built from Sean’s Indianapolis knowledge, current restaurant information, and places with a sustained local reputation.</p>
      <nav className="neighborhoodJump" aria-label="Jump to a neighborhood">
        {areas.map(area=><a key={area.id} href={'#'+area.id}>{area.name}</a>)}
      </nav>
    </section>

    <section className="contentPage neighborhoodGuide">
      {areas.map((area,index)=><section className="neighborhoodSection" id={area.id} key={area.id}>
        <div className="neighborhoodSectionHead">
          <p className="eyebrow">0{index+1} · FEATURED ANCHOR: {area.anchor}</p>
          <h2>{area.name}</h2>
          <p>{area.intro}</p>
        </div>
        <div className="restaurantPickGrid">
          {area.restaurants.map((restaurant,i)=><article className={i===0?"restaurantPick featuredPick":"restaurantPick"} key={restaurant.name}>
            <div className="pickTop"><span>{restaurant.label}</span><b>{restaurant.price}</b></div>
            <h3>{restaurant.name}</h3>
            <strong>{restaurant.best}</strong>
            <p>{restaurant.note}</p>
            <a href={restaurant.href} target="_blank" rel="noreferrer">Visit restaurant website ↗</a>
          </article>)}
        </div>
      </section>)}

      <aside className="neighborhoodMethod">
        <p className="eyebrow">HOW THESE PICKS WORK</p>
        <h2>Recommendations, not manufactured reviews.</h2>
        <p>Restaurants Sean has specifically identified are marked as anchor picks. The supporting choices are researched neighborhood recommendations—not claims that Sean personally reviewed every meal. As full BestaurantsIndy reviews are published, they will replace the external links.</p>
      </aside>
    </section>
    <SiteFooter/>
  </main>

...
export default function ReviewsExplorer(){
 ...
 return <><div className="reviewTools"><label><span>Search reviews</span><input value={query} onChange={e=>setQuery(e.target.value)} placeholder="Search by dish or cuisine"/></label><div className="filters">{["All","Japanese","Breakfast","Dinner"].map(x=><button className={filter===x?"active":""} onClick={()=>setFilter(x)} key={x}>{x}</button>)}</div></div>
 <div className="reviewGrid">{visible.map(x=><article key={x.title}><img src={x.image} alt=""/><div><p className="kicker">{x.category} · {x.verdict}</p><h2>{x.title}</h2><p>{x.summary}</p><a href={x.href}>Read field note →</a></div></article>)}</div>
